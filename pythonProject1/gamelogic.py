...
class GameLogic:
    ...
    def generate_words(self, size_grid: int, types: tuple):
        ...
        types_lines = []
        types_txt = {0: "animals.txt", 1: "biology.txt", 2: "physics.txt",
                     3: "plants.txt", 4: "professions.txt", 5: "sport.txt",
                     6: "technology.txt"}
            ...
            if merged < 5:
                for k in range(25):
                    ...
                    if dir_num == last_direction:
                        continue
                    dir_1 = {0: {"x": [0, size_grid - word_len],
                                 "y": [0, size_grid - 1]},
                             1: {"x": [0, size_grid - 1],
                                 "y": [0, size_grid - word_len]},
                             2: {"x": [0, size_grid - word_len],
                                 "y": [0, size_grid - word_len]},
                             3: {"x": [0, size_grid - word_len],
                                 "y": [word_len - 1, size_grid - 1]}}
                    x = random.randint(dir_1[dir_num]["x"][0],
                                       dir_1[dir_num]["x"][1])
                    y = random.randint(dir_1[dir_num]["y"][0],
                                       dir_1[dir_num]["y"][1])
                    ...
                    for i in range(len(words_grid)):
                        p2 = {"x": words_grid[i][0][0],
                              "y": words_grid[i][0][1]}
                        q2 = {"x": words_grid[i][1][0],
                              "y": words_grid[i][1][1]}
                ...
                for k in range(len(words)):
                    ...
                    for i in range(len(word)):
                        ...
                        for j in range(len(words[k])):
                            if words[k][j] != letter:
                                continue
                            dir_1 = {0: {"x": words_grid[k][0][0] + j,
                                         "y": words_grid[k][0][1]},
                                     1: {"x": words_grid[k][0][0],
                                         "y": words_grid[k][0][1] + j},
                                     2: {"x": words_grid[k][0][0] + j,
                                         "y": words_grid[k][0][1] + j},
                                     3: {"x": words_grid[k][0][0] + j,
                                         "y": words_grid[k][0][1] - j}}
                            letter_place = dir_1[words_grid[k][2]]
                            for dir_num in range(4):
                                if dir_num == words_grid[k][2]:
                                    continue
                                dir_2 = {0: {"x": letter_place["x"] - i,
                                             "y": letter_place["y"]},
                                         1: {"x": letter_place["x"],
                                             "y": letter_place["y"] - i},
                                         2: {"x": letter_place["x"] - i,
                                             "y": letter_place["y"] - i},
                                         3: {"x": letter_place["x"] - i,
                                             "y": letter_place["y"] + i}}
                                p1 = dir_2[dir_num]
                                dir_3 = {0: {"x": p1["x"] + word_len - 1,
                                             "y": p1["y"]},
                                         1: {"x": p1["x"],
                                             "y": p1["y"] + word_len - 1},
                                         2: {"x": p1["x"] + word_len - 1,
                                ...
                                for w in range(len(words_grid)):
                                    if w == k:
                                        continue
                                    p2 = {"x": words_grid[w][0][0],
                                          "y": words_grid[w][0][1]}
                                    q2 = {"x": words_grid[w][1][0],
                                          "y": words_grid[w][1][1]}
                                    if Intersect.do_intersect(p1, q1, p2, q2):
                                        flag = 0
                                        break
                                if flag == 1:
                                    ...
                                    dir_4 = {0: [p1["x"] + word_len - 1,
                                             ...
                                             3: [p1["x"] + word_len - 1,
                                                 p1["y"] - (word_len - 1)]}
                                    words_grid.append([[p1["x"], p1["y"]],
                                                       dir_4[dir_num], dir_num])
                                    self.words_left += 1
                                    exist_num.append([type_num, num])
                                    last_direction = dir_num
                                    retries = 0
    ...
    def place_words_on_grid(self, size_grid: int, words: list[str], words_grid):
        ...
        for k in range(len(words)):
            ...
            word_dir1 = {0: [word_grid[0][0], word_grid[1][0] + 1],
                         ...
                         2: [0, max(word_grid[1][0] + 1, word_grid[1][1] + 1) -
                             max(word_grid[0][0], word_grid[0][1])],
                         3: [0, min(word_grid[1][0] - word_grid[0][0] + 1,
                                    word_grid[0][1] - word_grid[1][1] + 1)]}
    ...
    def start_game(self, size_grid: int, types: tuple):
        ...
        return {"grid": grid, "words_left": self.words_left,
                "words_grid": words_grid}

    def return_statistic(self):
        return {"time": time.time() - self.start_time,
